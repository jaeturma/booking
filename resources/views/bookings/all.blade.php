<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
      All Bookings @if(!empty($officeName)) 
        <span class="ml-2 text-sm text-gray-500">— {{ $officeName }}</span> 
      @endif
    </h2>
  </x-slot>
    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="bg-white shadow-sm ring-1 ring-gray-200 sm:rounded-lg p-4">
        ...
        <div class="table-responsive">
          <table id="allBookingsTable" class="table table-sm table-striped table-hover table-bordered align-middle w-100">
            ...
            <tbody>
              @foreach($bookings as $b)
                <tr id="booking-row-{{ $b->id }}">
                  ...
                  <td data-order="{{ optional($b->created_at)->timestamp }}">
  ...
  @push('scripts')
    ...
    @include('partials.dt-init', [
      'tableId' => 'allBookingsTable',
      'orderCol' => 3,           // sort by Date/Time
      'noOrderTargets' => [-1],  // disable Action column sorting
